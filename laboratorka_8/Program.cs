...
namespace laboratorka_8
{
    class Program
    {
        ...
        static void Task3()
        {
            ...
            int Rosmir = Convert.ToInt32(Console.ReadLine());
            ...
            Func<int[], int> delet_na_seven = arr => arr.Count(x => x % 7 == 0);
            Console.WriteLine($"Кількість чисел, кратних 7: {delet_na_seven(masiv)}");
            Func<int[], int> Positiv = arr => arr.Count(x => x > 0);
            Console.WriteLine($"Кількість додатних чисел: {Positiv(masiv)}");
            Func<int, bool> isProgrammersDay = day => day == 256;
            Console.WriteLine($"Чи є 256 день програміста? {isProgrammersDay(256)}");
            Func<string, string[], bool> proverka = (text, words) => words.Any(word => text.Contains(word, StringComparison.OrdinalIgnoreCase));
